...
// upload thumbnail with nextmeal_thumb preset
export const uploadRecipeThumb = async file => {
  try {
    const data = new FormData();
    data.append('file', file);
    data.append('upload_preset', 'nextmeal_thumb');
...
// upload thumbnail with nextmeal_steps preset
export const uploadStepImage = async file => {
  try {
    const data = new FormData();
    data.append('file', file);
    data.append('upload_preset', 'nextmeal_steps');
    data.append('cloud_name', 'kazhala');
    const res = await fetch(CloudinaryURL, {
      method: 'POST',
      body: data
